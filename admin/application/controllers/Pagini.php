  ...
  public function Item()
  {
    ...
    switch($this->uriseg->item)
    {
        ...
        if(isset($_REQUEST["{$form_submititem}"])) {
				...
				/* form @structura */
					...
					$newDBPattern = (object) [ // Design Database Pattern
						...
						"database" => UPDATE,
						"type" => GET,
						"design" => array('is_active' => false)
					]; $update = $this->_Item->UPItem($newDBPattern->table, $viewdata["form"]->structura->prefix, $newDBPattern, $this->uriseg->id);// update@PageStructure
      ...
      break;
		...
		//breacrumb
		$breadcrumb = array("bb_titlu" => "Pagini site", "bb_button" => null, "breadcrumb" => array());
		
		$breadcrumb["breadcrumb"][0] = array("text" => "Pagini", "url" => '');
		$breadcrumb["breadcrumb"][1] = array("text" => $page["p"]->title, "url" => null);
    $view = (object) [ 'html' => array(
			0 => (object) ["viewhtml" => "layout/breadcrumb", "viewdata" => $breadcrumb],
      1 => (object) ["viewhtml" => "pagini/pagina", "viewdata" => $viewdata]
      ), 'javascript' => array(
	    ...
	    switch($this->uriseg->ajax)
			{
				...
				case BANNERFDATA:
					...
					if(isset($this->uriseg->ajax) && !is_null($this->uriseg->ajax)) {
						...
						$href = !empty($this->input->post("href")) ? trim($this->input->post("href")) : null;
						$thref = !empty($this->input->post("thref")) ? trim($this->input->post("thref")) : null;
						
						if(!is_null($ref)) {
							$data = array("titlu" => $ti, "subtitlu" => $sti, "href" => $href, "thref" => $thref);
