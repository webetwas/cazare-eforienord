...
class Legaturi extends CI_Controller {
  ...
  public function index()
  {
	...
	public function item()
	{
		...
		$viewdata = array("controller" => $this->controller, "item" => null, "parent" => null, "uri" => null, "form" => (object) []);
		$viewdata["uri"] = $this->uriseg;
        ...
        if(isset($_REQUEST["{$form_submititem}"])) {
			...
			case INSERT:
				...
				if(isset($_REQUEST["{$form_submititem}"])) {
					...
					if($insert) $this->_Session->setFB_Pozitive(array("ref" => "Legaturi", "text" => "Ai create legatura noua!"));
					redirect($this->controller. "/item/u/id/" .$insert);
				}
					
				if($parent) {
					$viewdata["form"]->item->segments .= "/parent/" .$this->uriseg->parent;//create2ndsegment
					
					$parent = $this->_Chain->getParentByIdLink(intval(trim($this->uriseg->parent)));//fetchparent
