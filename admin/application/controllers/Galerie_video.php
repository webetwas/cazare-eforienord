...
class Galerie_video extends CI_Controller {
  ...
  public function index()
  {
		...
		$viewdata["imgpathitem"] = SITE_URL.PATH_IMG_GALERIE_VIDEO."m/";
		...
		$breadcrumb["bb_button"] = array("text" => '<i class="fa fa-plus-square"></i> Adauga video', "linkhref" => $this->controller_fake ."/item/i");
		...
		switch($this->uriseg->item)
    {
      ...
      case INSERT:
        ...
        if(isset($_REQUEST["{$form_submititem}"])) {
					...
					if($insert) {
						$this->_Session->setFB_Pozitive(array("ref" => "Galerie video", "text" => "Ai adaugat un Video nou!"));
      ...
      break;
		...
		$breadcrumb["breadcrumb"][1] = array("text" => "Video", "url" => null);
    $view = (object) [ 'html' => array(
			0 => (object) ["viewhtml" => "layout/breadcrumb", "viewdata" => $breadcrumb],
				...
				case UPLOADIMG:
					...
					switch($this->input->post("filetarget"))
					{
						case UPIMGPOZA:
							...
							$locations->path = '../web/' .PATH_IMG_GALERIE_VIDEO;
