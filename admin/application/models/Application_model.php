...
class Application_model extends CI_Model {
  ...
  public function listPages($is_active) {
    $query = $this->db->query("
		SELECT {$this->tbl_pages}.id, {$this->tbl_pages}.id_page, {$this->tbl_pages}.position, {$this->tbl_pages_structure}.is_active, {$this->tbl_pages}.title, {$this->tbl_pages}.title_ro, {$this->tbl_pages}.title_browser_ro FROM `" .$this->tbl_pages. "`
		LEFT JOIN `{$this->tbl_pages_structure}` ON {$this->tbl_pages_structure}.idpage = {$this->tbl_pages}.id
		WHERE {$this->tbl_pages_structure}.is_active = " .$is_active. " ORDER BY {$this->tbl_pages}.position;");
