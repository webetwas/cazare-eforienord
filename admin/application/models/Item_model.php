...
class Item_model extends CI_Model {
  ...
  public function GetNewRTD($table, $querydata, $insertdata)
  {
	...
	public function getLastIdIncremented($table)
	{
  ...
  public function askLastItem($table = null) {
	...
	/**
	 * QuestionTrueFalse
	 * params String @column [column affected]
	 * params Array  @cxv    [column x value - whereto]
	 *
	 */
  public function QuestionTrueFalse($table, $column, $cxv) {
		
    $res = array("status" => 0, "value" => null);

    $query = $this->db->query("SELECT " .$column. " FROM `" .$table. "` WHERE {$cxv["column"]} = " .trim(intval($cxv["value"])). ";");
    if($query->num_rows() > 0) {
			
      if($query->row_array()[$column]) {
				
        $querychange = $this->db->query("UPDATE `" .$table. "` SET `" .trim($column). "` = 0 WHERE {$cxv["column"]} = " .trim(intval($cxv["value"])). ";");
        $value = 0;
      } else {
				
        $querychange = $this->db->query("UPDATE `" .$table. "` SET `" .trim($column). "` = 1 WHERE {$cxv["column"]} = " .trim(intval($cxv["value"])). ";");
        $value = 1;
      }
      if($this->db->affected_rows() > 0) {
